...
//    private static ExecutorService executor = Executors.newSingleThreadExecutor();
    ...
    public Pair<byte[], byte[]> hashimotoFull(BlockHeader header, long nonce) {
        return getEthashAlgo().hashimotoFull(getFullSize(), getFullDataset(), sha3(header.getEncoded()),
                longToBytes(nonce));
    }
        ...
        return new MineTask(block, nThreads,  new Callable<Long>() {
            ...
            public Long call() throws Exception {
                return getEthashAlgo().mine(getFullSize(), getFullDataset(),
                        sha3(block.getHeader().getEncoded()),
                        ByteUtil.byteArrayToLong(block.getCumulativeDifficulty().toByteArray()));
            }
        }).submit();
    }
